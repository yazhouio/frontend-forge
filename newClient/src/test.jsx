...
function Layout(props) {
      ...
      case "create-user":
        {
          ...
          if (method !== "GET") {
            return createUserMutate(() => fetch(url, {
              method,
              headers: {
  ...
  const dispatchActionCreateUserGraph = (actionId, event) => {
    ...
    switch (actionId) {
      case "INPUT_CHANGE":
        {
          nextContext = setActionCreateUserGraphPath(nextContext, "context.name", resolveActionCreateUserGraph("$event.value", event, nextContext));
          changed = true;
          break;
        }
      case "SUBMIT":
        {
          result = callActionCreateUserGraphDataSource("create-user", ["context.name"], event, nextContext);
          nextContext = setActionCreateUserGraphPath(nextContext, "context.name", "");
          changed = true;
          break;
        }
      default:
  ...
  return <div className='layout'><input value={actionCreateUserGraphContext.name ?? ""} placeholder={"Enter name"} onChange={event => {
      dispatchActionCreateUserGraph("INPUT_CHANGE", {
        value: event && event.target ? event.target.value : undefined,
        event
      });
    }} /><button className={"btn-primary"} disabled={false} onClick={event => {
