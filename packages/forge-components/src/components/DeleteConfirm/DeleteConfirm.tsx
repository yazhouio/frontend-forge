...
DeleteConfirmContentProps) {
  ...
  const resourceNames = resource?.map(
    (item: any) => item.delDisplayName ?? item.name,
  );
  ...
  const handleConfirmResourceChange = (
    ...
    <>
      ...
      <Footer>
        ...
        <StyledButton
          ...
          disabled={
            confirmLoading ||
            (resourceString ? resourceString !== confirmResource : false)
          }
          onClick={handleOk}
        >
          {t("OK")}
