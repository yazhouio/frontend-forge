...
export function requirePageSchema(body: PageSchemaRequestBody | unknown): unknown {
...
export function requireManifest(body: ProjectManifestRequestBody | unknown): ExtensionManifest {
...
export function requireScene(
  body: SceneRequestBody | unknown
): { type: SceneType; config: SceneConfig } {
  if (!body || typeof body !== 'object') {
  ...
  if (raw.type !== 'crdTable' && raw.type !== 'workspaceCrdTable' && raw.type !== 'iframe') {
    throw new ForgeError('scene.type must be "crdTable", "workspaceCrdTable", or "iframe"', 400);
  }
...
export function normalizeLabelValue(value: string, field: string): string {
  ...
  const stripped = replaced.replace(/^[^A-Za-z0-9]+/, '').replace(/[^A-Za-z0-9]+$/, '');
...
export function requireFrontendIntegration(
  ...
  if (integrationType !== 'crd' && integrationType !== 'iframe') {
    throw new ForgeError('spec.integration.type must be "crd" or "iframe"', 400);
  }
  ...
  if (integrationType === 'crd') {
    ...
    requireNonEmptyString(resourceObj.kind, 'spec.integration.crd.resource.kind');
    requireNonEmptyString(resourceObj.plural, 'spec.integration.crd.resource.plural');
    ...
    if (scope !== 'Namespaced' && scope !== 'Cluster') {
      throw new ForgeError('spec.integration.crd.scope must be "Namespaced" or "Cluster"', 400);
    }
  } else {
  ...
  const routingPath = requireNonEmptyString(routingObj.path, 'spec.routing.path');
  if (routingPath.startsWith('/')) {
    throw new ForgeError('spec.routing.path must not start with "/"', 400);
  }

  if (specObj.displayName !== undefined && typeof specObj.displayName !== 'string') {
  ...
  return {
    ...
    kind: typeof kindRaw === 'string' && kindRaw.trim().length > 0 ? kindRaw.trim() : 'FrontendIntegration',
...
export function parseFrontendIntegrationListQuery(query: FrontendIntegrationListQuery) {
