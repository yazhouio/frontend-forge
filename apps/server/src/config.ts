...
export const CACHE_DIR =
  process.env.CACHE_DIR || path.resolve(process.cwd(), ".cache");
export const CACHE_MAX_ITEMS = Number(process.env.CACHE_MAX_ITEMS || 200);
export const CONCURRENCY = Number(
  process.env.CONCURRENCY || Math.max(1, Math.floor(os.cpus().length / 2)),
);
...
export const CHILD_MAX_OLD_SPACE_MB = Number(
  process.env.CHILD_MAX_OLD_SPACE_MB || 512,
);
